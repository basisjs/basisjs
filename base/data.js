    ...
    var DataObject = Class(EventObject, {
      ...
      */
      init: function(config){
        ...
        if (!delegate && info instanceof DataObject)
          delegate = info;
      ...
      */
      setDelegate: function(delegate){
        ...
        if (this.canHaveDelegate && this.delegate !== delegate)
        {
          ...
          if (delegate && !this.isConnected(delegate))
          {
            // NOTE: test for cennected prevents from linking objects that had already connected (event through some other objects)
            ...
            this.setState(delegate.state, delegate.state.data); // test this changes
          ...
          else
          {
            ...
            if (oldDelegate)
            {
              ...
              for (var key in defaults)
              {
                if (key in delta)
                {
                  if (defaults[key] === delta[key])
                    delete delta[key];
                }
                else
                  delta[key] = undefined; //this.info[key];
