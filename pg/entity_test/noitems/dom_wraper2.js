 ...
 */
  ...
  (function(){
    ...
    * - Visual DOM classes:
    *   {Basis.DOM.Wrapers.HtmlNode}, {Basis.DOM.Wrapers.HtmlContainer}, 
    *   {Basis.DOM.Wrapers.HtmlPartitionNode}, {Basis.DOM.Wrapers.HtmlGroupControl},
    *   {Basis.DOM.Wrapers.HtmlControl}
    * - Misc:
    ...
    */
    var DATAOBJECT_DELEGATE_HANDLER = {
      ...
      datasetChanged: function(newValue, delta){ 
        ;;; if(typeof console != 'undefined') console.log('unnecessary event handler');
    ...
    */
    var AbstractNode = Class(DataObject, {
     ...
     /**
      * Map to collection items (collection item -> child node).
      * @type {Object}
      */
      collectionMap_: null,
      ...
      datasetChanged: function(newValue, delta){
        ...
        if (delta.deleted)
        {
          if (this.childNodes.length == delta.deleted.length)
          {
            ...
            this.clear();
            this.collection = col;
            con = this.collectionMap_ = {};
          }
          else
          {
            for (var i = 0, item; item = delta.deleted[i]; i++)
            {
              var node = con[item.eventObjectId];
              delete con[item.eventObjectId];
        ...
        if (delta.inserted)
        {
          for (var i = 0, item; item = delta.inserted[i]; i++)
          {
            ...
            con[item.eventObjectId] = true;
            con[item.eventObjectId] = this.insertBefore(item);
    ...
    var HierarchyTools = {
      ...
      */
      insertBefore: function(newChild, refChild){
        ...
        {
          if (this.collection && (!newChild || !this.collectionMap_[newChild.eventObjectId]))
        ...
        if (!isChildClassInstance)
        {
          ...
          if (newChild instanceof DataObject)
            newChild = {
      ...
      */
      removeChild: function(oldChild){
        ...
        if (this.collection && (!oldChild.delegate || this.collectionMap_[oldChild.delegate.eventObjectId]))
          throw new Error(EXCEPTION_COLLECTION_CONFLICT);
      ...
      */
      clear: function(alive){
        ...
        if (this.collection)
        {
          delete this.collectionMap_; // prevent node removing
          this.setCollection();
        }
      ...
      */
      setCollection: function(collection){
          ...
          if (this.collection)
          {
            ...
            if (this.collectionMap_)
            {
              if (this.childNodes.length == oldCollection.itemCount)
              {
                this.clear();
              }
              else
              {
                var nodes = Object.values(this.collectionMap_);
                delete this.collectionMap_;
          ...
          if (collection && this.canHaveChildren && collection instanceof DataObject)
          {
            ...
            //var collectionOrder = DOM.insert(this, this.collection.value/*.map(Data.wrapper('delegate'))*/);
            var nodes = this.collection.getItems();
            this.collectionMap_ = {};
            for (var i = 0; i < nodes.length; i++)
            {
              this.collectionMap_[nodes[i].eventObjectId] = true;
              this.collectionMap_[nodes[i].eventObjectId] = this.appendChild(nodes[i]);
            }

            //this.collectionMap_ = [];
            //this.collectionMap_.push.apply(this.collectionMap_, collectionOrder);
        ...
        if (!grouping)
        {
          if (this.groupControl)
          {
            ...
            if (this.firstChild)
            {
              ...
              if (this.localSorting)
                order = order.sortAsObject(this.localSorting, null, this.localSortingDesc);
    ...
    var HtmlNode = Class(Node, {
      ...
      */
      init: function(config){
        ...
        // insert content
        if (config.content)
          DOM.insert(this.content || this.element, config.content);
      ...
      insertBefore: function(newChild, refChild){
        ...
        { 
          if (this == newChild.parentNode)
          {
            ...
            if(!newChild.element || isNaN(newChild.element.nodeType) || (insertPoint && isNaN(insertPoint.nodeType)))debugger;
    ...
    */
    var HtmlGroupControl = Class(GroupControl, HtmlContainer).extend({
      ...
      behaviour: createBehaviour(HtmlContainer, GroupControl.prototype.behaviour),
