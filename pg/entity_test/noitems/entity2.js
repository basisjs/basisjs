    ...
    */
      ...
      if (this instanceof EntitySetWrapper)
      {
        ...
        return function(data, entitySet){
          if (data != null)
          {
            ...
            entitySet.set(data instanceof EntitySet ? data.getItems() : data);
    ...
    var EntitySet = Class(DataObject, {
      ...
      getItems: function(){
        ...
        if (!this.items_)
        {
          //console.log('getItems (calc)');
          this.items_ = Object.values(this.map_);
        }
        else
        {
          //console.log('getItems (cache)');
        }
      ...
      sync: function(data, set){
        ...
        var save = data.map(this.wrapper).sortAsObject(eventObjectIdGetter).unique(true);
        var self = this.getItems().sortAsObject(eventObjectIdGetter).unique(true);

        self.exclude(save).forEach(Data('destroy()'));
    ...
    function buffer_addItem(buffer, source, object){
      var objectId = object.eventObjectId;
    ...
    function buffer_removeItem(buffer, source, object){
      var objectId = object.eventObjectId;
      var sourceId = source.eventObjectId;
      var map_ = buffer.map_[objectId];
      ...
      datasetChanged: function(source, delta){
        ...
        if (delta.inserted)
        {
          for (var i = 0, item; item = delta.inserted[i]; i++)
            if (buffer_addItem(this, source, item))
              inserted.push(item);
        }
        ...
        if (delta.deleted)
        {
          for (var i = 0, item; item = delta.deleted[i]; i++)
            if (buffer_removeItem(this, source, item))
              deleted.push(item);
        }
    ...
    var DatasetBuffer = Class(EventObject, {
      ...
      getItems: function(){
        ...
        {
          //console.log('getItems (calc)');
          this.items_ = Object.values(this.map_).map(Data('obj'));
        }
        else
        {
          //console.log('getItems (cache)');
      ...
      add: function(source){
        if (source instanceof EntitySet)
        {
          ...
          if (!this.sourceMap_[objectId])
          {
            this.sourceMap_[objectId] = source;
            this.sources.push(source);

            source.addHandler(bufferHandler, this);
            bufferHandler.datasetChanged.call(this, source, { inserted: source.getItems() });
      ...
      remove: function(source){
        if (source instanceof EntitySet)
        {
          ...
          if (this.sourceMap_[objectId])
          {
            delete this.sourceMap_[objectId];
            this.sources.remove(source);

            source.removeHandler(bufferHandler, this);
            bufferHandler.datasetChanged.call(this, source, { deleted: source.getItems() });
    ...
    var EntitySetMerger = Class(EntitySet, {
      ...
      setFilter: function(filter){
        ...
        if (this.filter != filter)
        {
          ...
          var items = this.sources.getItems();
          var inserted = [];
          var deleted = [];
