...
Модели бывают простые, имеющие базовую функциональность. Такие модели могут иметь произвольный набор полей и значений.
...
var complex = SomeType({  // оператор new не указывается, так как все зависит от того,
...
[Скаляры](https://github.com/basisjs/articles/blob/master/ru-RU/basis.data.Value.md) хранят значения, которые воспринимаются как нечто неделимое. Самые простой класс умеет только хранить значение и вызывать обработчики при его изменении.
...
К этой группе так же относятся индексы, агрегированные значения от множества. Они автоматически обновляют сове значение при изменении элементов набора или его состава.
