...
На самом деле, фреймворку немало лет (около восьми), в течении которых он развивался как внутренний проект. Все это время, мы ждали хорошей альтернативы, которая предоcтавила бы сопостовимые возможности, подходы и инструменты. Но этого так и не случилось.
...
- поддержите "лайками" основной репозиторий и [другие репозитории проекта](https://github.com/basisjs), а так же публикации, которые вы встречаете о фреймворке – это поможет другим больше доверять проекту и более внимательно к нему присмотреться.

Проекту так же очень нужны:

- бесстрашные ниндзя, готовые стать первопроходцами и поделиться своим опытом, чем помогут фреймворк лучше и проще для начинающих;
...
Если речь про его исходные коды, то все, что хотите согласно лицензии `GPL` или `MIT`. Хотя это больше условность, так просто руководствуйтесь здравым смыслом.
...
- тест от Jeremy Ashkenas (создатель Backbone) с анимацией: [его оригинальное испольнение](http://jsfiddle.net/rdvornov/L46HM/) и [реализация, считающая полное время кадра](http://jsfiddle.net/rdvornov/yE9Z9/)/
...
По прежнему не знаете куда написать – пишите на `contact@basisjs.com`.
